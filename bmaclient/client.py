...
class MonitoringAPI(OAuth2API):
    ...
    )(bind_method(path="doas/", doc="Fetch DOAS data (legacy)."))

    fetch_gas_emission = bind_method(
        path="gas/emission/", doc="Fetch Vogamos gas emission data."
    )

    fetch_gas_temperature = bind_method(
        path="gas/temperature/", doc="Fetch Vogamos gas temperature data."
    )
    ...
    )(bind_method(path="thermal/", doc="Fetch thermal data (legacy)."))

    fetch_thermal2 = bind_method(path="thermal2/", doc="Fetch thermal data.")
    ...
    fetch_seismicity = bind_method(path="seismicity/", doc="Fetch seismicity data.")

    fetch_seismicity_archive = bind_method(
        path="seismicity/archive/", doc="Fetch seismicity archive data."
    )

    fetch_seismicity_cluster = bind_method(
        path="seismicity/cluster/", doc="Fetch seismicity cluster data."
    )

    fetch_bulletin = bind_method(path="bulletin/", doc="Fetch seismic bulletin data.")

    fetch_energy = bind_method(path="energy/", doc="Fetch seismic energy data.")

    fetch_magnitude = bind_method(
        path="magnitude/", doc="Fetch seismic magnitude data."
    )

    fetch_slope = bind_method(
        path="slope/", doc="Fetch EDM slope distance correction data."
    )
    ...
    create_slope = bind_method(
        path="slope/", method="POST", doc="Create EDM slope distance correction."
    )
    ...
    fetch_windrose = bind_method(
        path="meteorology/windrose/", doc="Fetch wind rose data."
    )

    fetch_rainfall = bind_method(
        path="meteorology/rainfall/", doc="Fetch rainfall data."
    )

    fetch_topo = bind_method(path="topo/", doc="Fetch topography data.")

    fetch_topo_profile = bind_method(
        path="topo/profile/", doc="Fetch topography profile data."
    )

    fetch_doas2 = bind_method(
        path="doas2/{station}/", accepts_parameters=["station"], doc="Fetch DOAS data."
    )

    fetch_csdr = bind_method(path="edm/csdr/", doc="Fetch EDM CSD and rate.")

    fetch_cluster_dict = bind_method(
        path="cluster/dict/", doc="Fetch cluster dictionary."
    )

    fetch_cluster_seisgroup = bind_method(
        path="cluster/seisgroup/", doc="Fetch cluster seismicity group."
    )
