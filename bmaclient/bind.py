...
class MonitoringAPIMethod(object):
    ...
    def _check_required_parameters(self, kwargs):
        for variable in self.required_parameters:
            if not kwargs.get(variable):
                raise APIClientError("Parameter '{}' is required.".format(variable))
    ...
    def _build_parameters(self, kwargs):
        ...
        for key, value in six.iteritems(kwargs):
            ...
            if key in self.parameters:
                raise APIClientError("Parameter '{}' already supplied.".format(key))
    ...
    def _do_api_request(self, url, method="GET", body=None, headers=None):
        ...
        if response["status"] in ERROR_STATUS:
            raise APIError(
                response["status"], ERROR_STATUS[response["status"]], content
            )
        ...
        if response["status"] == "200":
            ...
            raise APIError(response["status"], "HTTP_SERVICE_UNAVAILABLE", content)

    def execute(self):
        url, method, body, headers = Request(self.api).prepare_request(
